...
"""carto.py: utility module for Costom Carto py geoaxes to plot data on aacgmv2 coordinates."""
...
class SDCarto(GeoAxes):
    ...
    def overlay_data(
        ...
        **kwargs,
        ...
        df = df[
            (df.time >= self.plot_date)
            & (df.time < (self.plot_date + dt.timedelta(minutes=scan_time)))
        ]
